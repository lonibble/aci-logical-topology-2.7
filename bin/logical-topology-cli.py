    ...
    if len(index_list) > 0:
        if str(type(index_list)) == "<type 'dict'>":
            ...
            for k in sorted(index_list.keys()):
                for item in index_list[k].split(":::"):
            ...
            print "# {:>10.10} | {:85.85} #".format('Index', 'Value')
            print "#{:->100.100}#".format('')
            for index,items in enumerate(index_list):
...
def search_results():
    # search for a vlan or a port by string in session_vars['s_str'], use ":::" in the results as a line separator
    result_list = []
...
def menu_searchresult():
    ...
    local_choice = collect_string_input('h(elp)', 1, 3, '')
    return local_choice
# end of function menu_searchresult
...
json_file = script_dir+"../data/output-files/"+env+"/json-"+env+".json"
#
